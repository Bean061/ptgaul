...
echo "########## Step2 results ###########"
echo "####################################"
echo "filter_reads is your filtered sequence reads with all plastid \
...
if [ ! -s result_$FRL/new_filter_gt$FRL.fa ];then
	...
	echo "########## Step3 results ###########"
	echo "####################################"
	echo "new_filter_gt$FRL.fa is done. It keeps all plastid \
...
if [ ! -s result_$FRL/total_length ];then
	...
	echo $num
	if [[ $num = "" ]];then
		echo 'Step4 has been finished!! The coverage of plastome \
		is greater than 50x coverage.'
...
if [[ $number_edge -eq 3 ]]; then
	...
	and three lines in sorted_depth files."
	echo "then run python script as follows manually."
