...
echo "########## Step2 results ###########"
echo "####################################"
echo "filter_reads is your filtered sequence reads with all chloroplast \
seuqneces matching to the reference."
echo ""
...
if [ ! -s result_$FRL/new_filter_gt$FRL.fa ];then
	...
	echo "########## Step3 results ###########"
	echo "####################################"
	echo "new_filter_gt$FRL.fa is done. It keeps all chloroplast \
	long reads greater than $FRL bp."
...
if [ ! -s result_$FRL/total_length ];then
	...
	echo $num
	if [[ $num = "" ]];then
		echo 'Step4 has been finished!! The coverage of chloroplast \
		genome is greater than 50x coverage.'
		...
		if [ $frac != 0 ];then
			...
			echo 'Step5 has been finished!! The filter_reads_final.fa \
			includes about 50x coverage of long reads.'
		...
		echo 'Step4 has been finished!! The coverage of chloroplast genome \
		is between 15-50x coverage.'
		echo "It is ready for flye assembly"
...
if [ -s ./result_$FRL/flye_cpONT/assembly.fasta ];then
	...
	echo "You can use blast the aseembly sequence itself to check \
	the IR region, and use either long reads and short reads to check \
	the evenness of the coverages."
fi
...
if [[ $number_edge -eq 3 ]]; then
	...
	echo '==============================================='
	echo "The gfa file has 3 edges. It is ready for the combine_gfa.py script. \
	...
	echo "After confirming the assembly result, if the result is a circle."
	...
	echo '==============================================='
	echo "The gfa file has more than/less than three edges. Please manually \
	check the gfa file in Bandage."
	echo "After confirming the assembly result, keep three edges in the edges.fa \
	and sorted_depth files."
	echo "./result_$FRL/flye_cpONT/assembly.fasta is your final result."
